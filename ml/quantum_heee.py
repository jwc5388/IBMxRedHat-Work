...
# ──────────────────────────────────────────────────────────────────────────────
base_path = '/Users/jaewoo000/Desktop/IBM:RedHat/Study25/_data/quantum2/'
...
GPU_NUM = 3  # 사용할 GPU 번호 설정

if torch.cuda.is_available():
    device = torch.device(f'cuda:{GPU_NUM}')
    print(f"현재 사용 중인 GPU 번호: {GPU_NUM}")
    print(f"GPU 이름: {torch.cuda.get_device_name(GPU_NUM)}")
    print(f"GPU 총 개수: {torch.cuda.device_count()}")
else:
